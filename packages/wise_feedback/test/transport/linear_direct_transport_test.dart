...
void main() {
  group('LinearDirectTransport', () {
    test(
      'runs fileUpload -> PUT -> issueCreate and returns issue info',
      () async {
        final calls = <String>[];
    ...
    test(
      'throws FeedbackException when fileUpload returns no upload url',
      () async {
    ...
    test(
      'throws FeedbackException when issueCreate reports success: false',
      () async {
        final client = MockClient((request) async {
          if (request.method == 'POST') {
            final body = jsonDecode(request.body) as Map<String, dynamic>;
